...
workflow RIBOSEQ {
    ...
    if (!params.skip_premap) {
        ...
        ch_logs = FASTQC.out.html.join(FASTQC.out.zip)
            .map { [ it[1], it[2] ] }
            .collect()
            .mix(PREMAP.out.log.collect(), MAP.out.log.collect(), PCA.out.pca_mqc, RIBOSEQ_QC.out.fq_length_mqc.collect(), RUN_RIBOCUTTER.out.ribocutter_mqc, SUMMARISE_RIBOSEQ_QC.out.pcoding_percentage_mqc, SUMMARISE_RIBOSEQ_QC.out.expected_length_mqc, SUMMARISE_RIBOSEQ_QC.out.duplication_mqc)
            .collect()
    } else {
        ...
        ch_logs = FASTQC.out.html.join(FASTQC.out.zip)
            .map { [ it[1], it[2] ] }
            .collect()
            .mix(MAP.out.log.collect(), PCA.out.pca_mqc, RIBOSEQ_QC.out.fq_length_mqc.collect(), RUN_RIBOCUTTER.out.ribocutter_mqc, SUMMARISE_RIBOSEQ_QC.out.pcoding_percentage_mqc, SUMMARISE_RIBOSEQ_QC.out.expected_length_mqc, SUMMARISE_RIBOSEQ_QC.out.duplication_mqc)
            .collect()
