...
workflow {
    ...
    ch_logs = FASTQC.out.fastqc.collect().mix(PREMAP.out.log.collect().ifEmpty([]), MAP.out.log.collect(), CUTADAPT.out.log.collect().ifEmpty([]), DEDUPLICATE.out.log.collect().ifEmpty([]))
