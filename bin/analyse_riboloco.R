...
option_list <- list(make_option(c("-i", "--input"), action = "store", type = "character", default=NA, help = "riboloco-lite output"),
                    ...
                    make_option(c("-o", "--output_prefix"), action = "store", type = "integer", default=3, help = "output prefix"))
...
info <- read_csv(opt$transcript_info, delim="\t")

lengths_of_interest <- eval(parse(text = opt$lengths_of_interest))
...
#### RUN ####
df <- read_csv(riboloco_lite_output) %>%
...
p1 <- ggplot(df2 %>% filter(abs(downsampled_pos) < 200/downsample_for_heatmaps), 
...
p2 <- ggplot(df2 %>% filter(abs(downsampled_pos) < 200/downsample_for_heatmaps) %>% distinct(name, downsampled_pos, n_footprints_total),
...
ggsave(paste0(prefix,'.footprint_heatmap.pdf'),
       p4 , height = 25, width = 20, units = 'cm')


ggsave('~/Documents/GitHub/riboloco_lite/kl_div_rank_plot.pdf',
...
df4 <- bind_rows(df3_0, df3_1) %>% bind_rows(df3_2) %>%
...
ggsave(paste0(prefix, '.kl_div_decrease.pdf'),
       drop.gg, height = 20, width = 40, units = 'cm')
