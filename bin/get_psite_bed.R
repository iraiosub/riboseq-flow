...
table_to_granges <- function(df) {
    ...
    df <- df %>%
        ...
        mutate(strand = "*", start = psite, end = psite, score = 1)

    df.gr <- GRanges(df)
    names(df.gr) <- df.gr$transcript


    # Add up scores for identical positions
...
# A function that converts transcript to genomic coordinates
convert_coordinates <- function(tx.gr, annotation) {
...
exons.grl <- exons.grl[names %in% unique(rpf.df$transcript)]

# Get transcriptomic GRanges
rpf.tx.grl <- lapply(psites.df.ls, table_to_granges)

# Mapping transcript coordinates to genomic coordinates
rpf.genomic.grl <- lapply(rpf.tx.grl, convert_coordinates, annotation = exons.grl )
